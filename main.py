...
def tower_text(game: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"Ставка: <b>{fmt_money(bet)}</b>\n"
        f"Этаж: <b>{level}</b>\n"
        f"РўРµРєСѓС‰РёР№ РјРЅРѕР¶РёС‚РµР»ь: <b>x{current_mult:.2f}</b>\n"
        f"РџРѕС‚РµРЅС†РёР°Р»ьно СЃРµР№С‡Р°с: <b>{fmt_money(current_win)}</b>\n"
        f"РЎР»РµРґСѓСЋС‰РёР№ этаж: <b>x{next_mult:.2f}</b>\n\n"
        "Р’С‹Р±Рµри одну из 3 СЃРµРєС†РёР№."
    )
...
def gold_text(game: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"Ставка: <b>{fmt_money(bet)}</b>\n"
        f"Р Р°унд: <b>{step}</b>\n"
        f"РўРµРєСѓС‰РёР№ РјРЅРѕР¶РёС‚РµР»ь: <b>x{cur_mult:.2f}</b>\n"
        f"РџРѕС‚РµРЅС†РёР°Р»ьно СЃРµР№С‡Р°с: <b>{fmt_money(cur_win)}</b>\n"
        f"РЎР»РµРґСѓСЋС‰РёР№ СЂР°унд: <b>x{next_mult:.2f}</b>\n\n"
        "Р’С‹Р±Рµри РїР»РёС‚ку (одна из них с Р»РѕРІСѓС€РєРѕР№)."
    )
...
async def gold_pick(query: CallbackQuery):
    ...
    if chosen == trap:
        ...
        await query.message.edit_text(
            "💥 <b>Р—РѕР»РѕС‚о</b>\n"
            f"Р›РѕРІСѓС€ка в плитке <b>{trap}</b>.\n"
            f"РџРѕС‚Рµряно: <b>{fmt_money(bet)}</b>\n"
            f"Баланс: <b>{fmt_money(balance)}</b>"
        )
        await query.answer()
        return
...
async def gold_cash(query: CallbackQuery):
    ...
    bet = float(game["bet"])
    if step <= 0:
        await query.answer("Сначала СЃРґРµР»Р°Р№ минимум 1 С…од", show_alert=True)
        return
    ...
    await query.message.edit_text(
        "✅ <b>Золото: выигрыш</b>\n"
        f"Р Р°унд: <b>{step}</b>\n"
        f"РњРЅРѕР¶РёС‚РµР»ь: <b>x{mult:.2f}</b>\n"
        f"Выплата: <b>{fmt_money(payout)}</b>\n"
        f"Баланс: <b>{fmt_money(balance)}</b>"
    )
    await query.answer()
...
async def gold_cancel(query: CallbackQuery):
    ...
    await query.message.edit_text(
        "❌ <b>Р—РѕР»РѕС‚о Р·Р°РІРµСЂС€Рµно</b>\n"
        f"Возврат: <b>{fmt_money(payout)}</b>\n"
        f"Баланс: <b>{fmt_money(balance)}</b>"
    )
    await query.answer()
...
def diamond_text(game: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"РўРµРєСѓС‰РёР№ РјРЅРѕР¶РёС‚РµР»ь: <b>x{cur_mult:.2f}</b>\n"
        f"РџРѕС‚РµРЅС†РёР°Р»ьно СЃРµР№С‡Р°с: <b>{fmt_money(cur_win)}</b>\n"
        f"РЎР»РµРґСѓСЋС‰РёР№ С€Р°г: <b>x{next_mult:.2f}</b>\n\n"
        "Р’С‹Р±Рµри кристалл (один из них Р±СЂР°РєРѕРІР°РЅРЅС‹Р№)."
    )
...
async def diamond_pick(query: CallbackQuery):
    ...
    if chosen == trap:
        ...
        await query.message.edit_text(
            "💥 <b>Алмазы</b>\n"
            f"Р‘СЂР°РєРѕРІР°РЅРЅС‹Р№ кристалл: <b>{trap}</b>.\n"
            f"РџРѕС‚Рµряно: <b>{fmt_money(bet)}</b>\n"
            f"Баланс: <b>{fmt_money(balance)}</b>"
        )
        await query.answer()
    ...
    if step >= len(DIAMOND_MULTIPLIERS):
        ...
        await query.message.edit_text(
            "🏁 <b>Алмазы</b>\n"
            "РњР°ксимум С€Р°гов РїСЂРѕР№РґРµн.\n"
            f"Выплата: <b>{fmt_money(payout)}</b>\n"
            f"Баланс: <b>{fmt_money(balance)}</b>"
        )
        await query.answer()
...
async def diamond_cash(query: CallbackQuery):
    ...
    bet = float(game["bet"])
    if step <= 0:
        await query.answer("Сначала СЃРґРµР»Р°Р№ минимум 1 С€Р°г", show_alert=True)
        return
...
def nmines_keyboard(
    ...
    if opened > 0:
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    text=f"Р—Р°Р±СЂР°С‚и {fmt_money(potential)}",
...
async def legacy_diamonds_cb(query: CallbackQuery):
    ...
    if not state:
        return await query.answer("Р“ру Р·Р°РІРµСЂС€Рµно", show_alert=True)
    if int(state["uid"]) != query.from_user.id:
        return await query.answer("Це не С‚воя гра", show_alert=True)
